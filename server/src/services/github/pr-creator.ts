...
export async function createGitHubPR(options: CreatePROptions): Promise<{ prUrl: string; prNumber: number } | null> {
  ...
  try {
    ...
    // 3. Create new branch
    const branchName = remediation.branch || `fix/buzrig-${remediationId.slice(0, 8)}`
    try {
    ...
    if (files.length === 0) {
      ...
      files.push({ path: 'BUZRIG_FIX.patch', content: remediation.patch })
    }
